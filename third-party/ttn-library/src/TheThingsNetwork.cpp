...
const char *const error_msg[] PROGMEM = { invalid_sf,       invalid_fp,         unexpected_response,      send_command_failed,
                                          join_failed,      join_not_accepted,  personalize_not_accepted, response_is_not_ok,
                                          error_key_length, check_configuration };
...
const char *const radio_table[] PROGMEM = { radio_prefix,    radio_set,     radio_get,    radio_get_bw,
                                            radio_get_prlen, radio_get_crc, radio_get_cr, radio_get_sf };
...
const char *const sys_table[] PROGMEM = { sys_prefix, sys_sleep,   sys_reset,   sys_erase_fw,  sys_factory_rst, sys_set,
                                          sys_get,    sys_get_ver, sys_get_vdd, sys_get_hweui, sys_set_get_nvm, sys_set_pindig };
...
const char *const mac_table[] PROGMEM = { mac_prefix,       mac_reset, mac_tx,     mac_join, mac_save,
                                          mac_force_enable, mac_pause, mac_resume, mac_set,  mac_get };
...
const char *const mac_options[] PROGMEM = { mac_devaddr, mac_deveui, mac_appeui, mac_nwkskey, mac_appskey, mac_appkey,   mac_pwridx,
                                            mac_dr,      mac_adr,    mac_bat,    mac_retx,    mac_linkchk, mac_rxdelay1, mac_rxdelay2,
                                            mac_band,    mac_ar,     mac_rx2,    mac_ch,      mac_gwnb,    mac_mrgn,     mac_class };
...
int pgmstrcmp(const char *str1, uint8_t str2Index) {
    char str2[128];
    strcpy_P(str2, (char *)fk_pgm_read_word(&(compare_table[str2Index])));
...
void TheThingsNetwork::debugPrintIndex(uint8_t index, const char *value) {
    char message[100];
    strcpy_P(message, (char *)fk_pgm_read_word(&(show_table[index])));
    debugPrint(message);
    if (value) {
        debugPrintLn(value);
    }
...
void TheThingsNetwork::debugPrintMessage(uint8_t type, uint8_t index, const char *value) {
    ...
    switch (type) {
    case ERR_MESSAGE:
        strcpy_P(message, (char *)fk_pgm_read_word(&(error_msg[index])));
        break;
    case SUCCESS_MESSAGE:
        strcpy_P(message, (char *)fk_pgm_read_word(&(success_msg[index])));
        break;
    }
    debugPrint(message);
    if (value) {
...
void TheThingsNetwork::sendCommand(uint8_t table, uint8_t index, bool appendSpace, bool print) {
    ...
    switch (table) {
    case MAC_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(mac_table[index])));
        break;
    case MAC_GET_SET_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(mac_options[index])));
        break;
    case MAC_JOIN_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(mac_join_mode[index])));
        break;
    case MAC_CH_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(mac_ch_options[index])));
        break;
    case MAC_TX_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(mac_tx_table[index])));
        break;
    case SYS_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(sys_table[index])));
        break;
    case RADIO_TABLE:
        strcpy_P(command, (char *)fk_pgm_read_word(&(radio_table[index])));
        break;
    default:
        return;
    }
