...
static void task_handler_reader(void *params) {
    ...
    while (true) {
        ...
        if (os_rwlock_acquire_read(os_rwlock(rwlock), 250) == OSS_SUCCESS) {
            ...
            os_printf(RTT_CTRL_TEXT_CYAN "%s: success (%dms) r=%lu w=%lu\n" RTT_CTRL_RESET, os_task_name(), wms, os_rwlock(rwlock)->readers,
                      os_rwlock(rwlock)->writers);
            os_delay(wms);
...
static void task_handler_writer(void *params) {
    ...
    while (true) {
        ...
        if (os_rwlock_acquire_write(os_rwlock(rwlock), 250) == OSS_SUCCESS) {
            ...
            auto elapsed = os_uptime() - started;
            os_printf("%s: fail (after %lums) r=%lu w=%lu\n" RTT_CTRL_RESET, os_task_name(), elapsed, os_rwlock(rwlock)->readers,
                      os_rwlock(rwlock)->writers);
        }
...
void setup() {
    ...
    for (auto i = 0; i < NUMBER_OF_READERS; ++i) {
        ...
        OS_CHECK(os_task_initialize(&reader_tasks[i], strdup(temp), OS_TASK_START_RUNNING, &task_handler_reader, NULL, reader_stacks[i],
                                    sizeof(reader_stacks[i])));
    }

    for (auto i = 0; i < NUMBER_OF_WRITERS; ++i) {
        char temp[32];
        os_snprintf(temp, sizeof(temp), "writer-%d", i);
        OS_CHECK(os_task_initialize(&writer_tasks[i], strdup(temp), OS_TASK_START_RUNNING, &task_handler_writer, NULL, writer_stacks[i],
                                    sizeof(writer_stacks[i])));
    }
