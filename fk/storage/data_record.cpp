...
void DataRecord::include_readings(GlobalState const *gs, fkb_header_t const *fkb_header, uint32_t meta_record, Pool &pool) {
    ...
    if (gs->modules != nullptr) {
        ...
        for (auto i = 0u; i < nmodules; ++i) {
            auto &module_state = gs->modules->modules[i];
            auto position = module_state.position;
            auto index = position.integer();
            auto &physical_module_state = gs->physical_modules[index];
            auto meta = physical_module_state.meta;

            auto nsensors = module_state.nsensors;
            auto sensor_values = nsensors > 0 ? pool.malloc<fk_data_SensorAndValue>(nsensors) : nullptr;
            for (auto sensor_index = 0u; sensor_index < nsensors; ++sensor_index) {
                auto &sensor_state = module_state.sensors[sensor_index];
                auto reading = sensor_state.live_value;

                loginfo("bay[%d] sensor[%2d] '%s.%s' = %f (%f)", position.integer(), sensor_index, meta->name,
                        sensor_state.name, reading.calibrated, reading.uncalibrated);
                sensor_values[sensor_index] = fk_data_SensorAndValue_init_default;
            ...
            auto readings_array = pool.malloc<pb_array_t>();
            readings_array->length = nsensors;
            readings_array->itemSize = sizeof(fk_data_SensorAndValue);
            readings_array->buffer = sensor_values;
            readings_array->fields = fk_data_SensorAndValue_fields;
