...
int32_t File::write_record_header(RecordSize size) {
    auto g = geometry();
    ...
    log_hashed_data(FK_OP_STR_WRITE, FK_OP_STR_HEADER, file_, record_ - 1, tail_, &record_header, sizeof(record_header));
...
int32_t File::write_partial(uint8_t const *record, size_t size) {
    ...
    logverbose("[%d] " PRADDRESS " write data (%zd bytes) (%" PRIu32 " lib)", file_, tail_, size, left_in_block);
    ...
    log_hashed_data(FK_OP_STR_WRITE, FK_OP_STR_DATA, file_, record_ - 1, tail_, record, size);
...
int32_t File::write_record_tail(RecordSize size) {
...
uint32_t File::find_previous_sector_aligned_record(StorageAddress address) {
    auto g = storage_->geometry();
...
int32_t File::read(uint8_t *record, size_t size) {
    ...
    while (bytes_read < size) {
        if (record_remaining_ == 0) {
            ...
            log_hashed_data(FK_OP_STR_READ, FK_OP_STR_DATA, file_, record_, tail_, record + bytes_read, reading);
