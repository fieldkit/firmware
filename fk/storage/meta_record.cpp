...
void MetaRecord::include_modules(GlobalState const *gs, fkb_header_t const *fkb_header, Pool &pool) {
    ...
    for (auto i = 0u; i < gs->modules->nmodules; ++i) {
        ...
        auto constructed_module = get_module_factory().get(position);
        if (!constructed_module) {
            logerror("constructed module");
            continue;
        }

        auto header = (*constructed_module)->found.header;
        auto configuration = (*constructed_module)->configuration;
        auto module_instance = (*constructed_module)->module_instance;
