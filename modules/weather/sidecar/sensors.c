...
int32_t sht31_initialize(struct i2c_m_sync_desc *i2c) {
    ...
    rv = write_command_u16(i2c, SHT31_I2C_ADDRESS, SHT31_SOFT_RESET);
    if (rv != 0) {
        return rv;
    }
...
int32_t sht31_status_get(struct i2c_m_sync_desc *i2c, uint16_t *status) {
    ...
    rv = write_command_u16(i2c, SHT31_I2C_ADDRESS, SHT31_READ_STATUS);
    if (rv != 0) {
        return rv;
    }
...
int32_t sht31_reading_get(struct i2c_m_sync_desc *i2c, sht31_reading_t *reading) {
    ...
    rv = write_command_u16(i2c, SHT31_I2C_ADDRESS, SHT31_MEAS_HIGHREP);
    if (rv != 0) {
        return rv;
    }
...
int32_t mpl3115a2_initialize(struct i2c_m_sync_desc *i2c) {
    ...
    i2c_m_sync_set_slaveaddr(i2c, MPL3115A2_I2C_ADDRESS, I2C_M_SEVEN);

    uint8_t reg = MPL3115A2_WHOAMI;

    struct _i2c_m_msg msg;
    msg.addr   = MPL3115A2_I2C_ADDRESS;
    msg.flags  = I2C_M_SEVEN;
    msg.buffer = (void *)&reg;
    msg.len    = sizeof(reg);
    rv = _i2c_m_sync_transfer(&i2c->device, &msg);
...
int32_t adc081c_initialize(struct i2c_m_sync_desc *i2c) {
    ...
    i2c_m_sync_set_slaveaddr(i2c, ADC081C_I2C_ADDRESS, I2C_M_SEVEN);

    uint8_t reg = ADC081C_REGISTER_CONFIGURATION;

    struct _i2c_m_msg msg;
    msg.addr   = ADC081C_I2C_ADDRESS;
    msg.flags  = I2C_M_SEVEN;
    msg.buffer = (void *)&reg;
    msg.len    = sizeof(reg);
    rv = _i2c_m_sync_transfer(&i2c->device, &msg);
...
int32_t adc081c_reading_get(struct i2c_m_sync_desc *i2c, adc081c_reading_t *reading) {
    ...
    uint8_t reg = ADC081C_REGISTER_READING;
    ...
    msg.flags  = I2C_M_STOP | I2C_M_RD | I2C_M_SEVEN;
    msg.buffer = (void *)&reading->value;
    msg.len    = sizeof(reading->value);
    rv = _i2c_m_sync_transfer(&i2c->device, &msg);
    if (rv != 0) {
        return rv;
    }
